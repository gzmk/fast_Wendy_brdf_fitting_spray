...
ro_d = ['300:', num2str(var(1)), ' 800:', num2str(var(1))];
% alphau = var(2); % alphau and alphav should always be the same value for isotropic brdf
light = ['300:', num2str(1), ' 800:',num2str(1)];
mycell = {ro_d,light};

T = cell2table(mycell, 'VariableNames', {'ro_d' 'light'});
writetable(T,'/scratch/gk925/fast_Wendy_brdf_fitting_spray/diffuse_gloss0_fit/sphere_3params_Conditions.txt','Delimiter','\t')
%% Rendering bit

% Set preferences
setpref('RenderToolbox3', 'workingFolder', '/scratch/gk925/fast_Wendy_brdf_fitting_spray/diffuse_gloss0_fit');
...
imPath = ['/scratch/gk925/fast_Wendy_brdf_fitting_spray/diffuse_gloss0_fit/', hints.recipeName, '/renderings/Mitsuba/test_sphere-001.mat']
